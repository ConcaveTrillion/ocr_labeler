...
class LineOperations:
    ...
    def split_lines_with_selected_words(
        self, page: "Page", word_keys: list[tuple[int, int]]
    ) -> bool:
        """Split each selected line into selected-word and unselected-word lines.

        Args:
            page: Page containing lines/words.
            word_keys: List of ``(line_index, word_index)`` selections.

        Returns:
            bool: True when split succeeds for all targeted lines, False otherwise.
        """
        if not page:
            logger.warning("No page provided for split-lines-with-selected-words")
            return False
        ...
        try:
            ...
            if not unique_keys:
                logger.warning(
                    "Split-lines-with-selected-words requires selecting at least one word"
                )
                return False

            lines = list(getattr(page, "lines", []) or [])
            selected_indices_by_line: dict[int, set[int]] = {}

            for line_index, word_index in unique_keys:
                if line_index < 0 or line_index >= len(lines):
                    logger.warning(
                        "Line index %s out of range for split-lines-with-selected-words (0-%s)",
                        line_index,
                        len(lines) - 1,
                    )
                    return False
                ...
                selected_indices_by_line.setdefault(line_index, set()).add(word_index)

            split_words_by_line_id: dict[int, tuple[list[object], list[object]]] = {}
            for line_index, selected_word_indices in selected_indices_by_line.items():
                target_line = lines[line_index]
                line_words = list(getattr(target_line, "words", []) or [])

                if len(line_words) < 2:
                    logger.warning(
                        "Line %s has fewer than two words; cannot split by selected words",
                        line_index,
                    )
                    return False
                ...
                unselected_words = [
                    word
                    for index, word in enumerate(line_words)
                    if index not in selected_word_indices
                ]

                if not selected_words or not unselected_words:
                    logger.warning(
                        "Line %s requires a strict subset of selected words for split",
                        line_index,
                    )
                    return False

                split_words_by_line_id[id(target_line)] = (
                    selected_words,
                    unselected_words,
                )

            paragraphs = list(getattr(page, "paragraphs", []) or [])
            found_line_ids: set[int] = set()
            for paragraph in paragraphs:
                paragraph_lines = list(getattr(paragraph, "lines", []) or [])
                for line in paragraph_lines:
                    line_id = id(line)
                    if line_id in split_words_by_line_id:
                        found_line_ids.add(line_id)

            expected_line_ids = set(split_words_by_line_id.keys())
            if found_line_ids != expected_line_ids:
                logger.warning(
                    "Unable to locate all selected lines in paragraphs for split-lines-with-selected-words"
                )
                return False

            any_changes = False
            for paragraph in paragraphs:
                paragraph_items = list(getattr(paragraph, "items", []) or [])
                new_items: list[object] = []
                paragraph_changed = False

                for line in paragraph_items:
                    split_words = split_words_by_line_id.get(id(line))
                    if split_words is None:
                        new_items.append(line)
                        continue

                    selected_words, unselected_words = split_words
                    line.items = selected_words
                    line.unmatched_ground_truth_words = []
                    line.recompute_bounding_box()

                    unselected_line = Block(
                        items=unselected_words,
                        child_type=BlockChildType.WORDS,
                        block_category=BlockCategory.LINE,
                    )
                    new_items.extend([line, unselected_line])
                    paragraph_changed = True

                if paragraph_changed:
                    paragraph.items = new_items
                    paragraph.recompute_bounding_box()
                    any_changes = True

            if not any_changes:
                logger.warning(
                    "No paragraph updates applied for split-lines-with-selected-words"
                )
                return False
            ...
            logger.info(
                "Split selected words into selected/unselected lines for line indices %s",
                sorted(selected_indices_by_line.keys()),
            )
            return True

        except Exception as e:
            logger.exception(
                "Error splitting lines with selected words %s: %s",
                word_keys,
                e,
            )
            return False
