...
class NavigationOperations:
    ...
    def goto_page_index(
        ...
        if max_index < 0:
            logger.debug("goto_page_index: no pages available")
            return NavigationResult(False, "No pages available"), -1

        # Clamp to valid range
        if target_index < 0:
            logger.debug("goto_page_index: clamping %s -> 0", target_index)
            clamped_index = 0
        elif target_index > max_index:
            logger.debug("goto_page_index: clamping %s -> %s", target_index, max_index)
