"""Tests for save_page functionality in ProjectState."""
...
class TestSavePage:
    ...
    def test_save_page_with_valid_page(self, tmp_path):
        ...
        result = self.project_state.save_page(
            page=mock_page, save_directory=str(save_dir), project_id="test_project"
        )
    ...
    def test_save_page_no_current_page(self, tmp_path):
        """Test save_page when no current page is available."""
        self.project_state.current_page_native = None
    ...
    def test_save_page_generates_project_id_from_root(self, tmp_path):
        ...
        # Set project_root name
        self.project_state.project_root = Path("my_book_project")

        save_dir = tmp_path / "save_test"
        result = self.project_state.save_page(
            page=mock_page, save_directory=str(save_dir)
        )
    ...
    def test_save_page_handles_different_image_extensions(self, tmp_path):
        ...
        for ext in [".png", ".jpg", ".jpeg"]:
            ...
            result = self.project_state.save_page(
                page=mock_page, save_directory=str(save_dir), project_id="test"
            )
    ...
    def test_save_page_missing_image_path(self, tmp_path):
        ...
        result = self.project_state.save_page(
            page=mock_page, save_directory=str(tmp_path / "save_test")
        )
